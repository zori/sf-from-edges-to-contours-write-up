...
easily satisfiable by using a data structure called an {\it Ultra metric contour map} ({\tt UCM}) \cite{Arbelaez2006boundary}. While details will be given in \sref{sec:ch3-UCM}, for now it is sufficient to know that it represents %encapsulates 
...
\paragraph{Example}\mbox{}\\\mbox{}\\
In \fref{fig:segmentation} we show the result of the algorithm {\tt gPb-OWT-UCM} \cite{Arbelaez09,Arbelaez11}, which we will present in \cref{Chapter3}. The output is a {\tt UCM} - see \fref{fig:sub:segmentation-ucm} for an example of UCM output and \fref{fig:sub:segmentation-ucm-cc} for its colour-coded visualisation.
...
the effectiveness of the above detectors. More higher-level techniques incorporate colour and texture \cite{Rubner1996coalescing,Will2000learning,Malik2001contour} information, as well as multiple scales \cite{Martin2004learning} and orientations \cite{Perona1990detecting,Freeman1991design}. % TODO examples
...
\paragraph{Crisp boundary detection using \textit{pointwise mutual information} ({\tt PMI}):} The method, due to Isola \etal \cite{Isola2014crisp}, utilises internal image statistics to model the presence of an edge. It derives an affinity measure from {\tt PMI}, which is a good predictor of whether or not two pixels reside on the same object. The authors then reuse the spectral partitioning machinery which is part of the {\tt gPb} algorithm (to be explained in \sref{sec:ch3-gPb}). The method produces pixel-level accurate (\ie, {\it crisp}) edges. %boundaries.
...
The following is by no means a comprehensive survey of image segmentation algorithms, as the problem is old and there exist many and varied approaches to solving it. Instead, it provides a  selection of 1) highly relevant to our proposed approach, and\slash or 2) current state-of-the art methods.
...
tendency to produce oversegmentations from natural images.

\subsubsection{Region growing}
The {\bf seeded} region growing algorithm \cite{Adams1994seeded} takes as input the image and a set of seeds. The seeds mark each of the objects to be segmented. The regions are iteratively grown by comparing all unallocated neighbouring pixels to the regions. {\bf Unseeded} region growing \cite{Lin2000unseeded} was subsequently developed, which makes use of the Perona-Malik anisotropic diffusion \cite{Perona1990scale}. Although it allows to forgo the choice of seeds (only one is required), it still has problems like proper parameter choice % threshold 
...
The algorithm {\tt gPb-OWT-UCM} \cite{Arbelaez09,Arbelaez11} is an elaborate pipeline that starts with edge extraction ({\tt gPb}) to obtain in the end a set of segmentations at hierarchical scales, represented by a {\tt UCM}. We used it to illustrate segmentation-related terminology in \fref{fig:segmentation} and we will describe it in detail in \cref{Chapter3}.
...
It begins by independently performing a hierarchical segmentation at each scale of a multiresolution image pyramid. Afterwards, the multiple hierarchies are aligned and combined into a single multiscale segmentation hierarchy.
...
\section{Outline}
The rest of this work is structured as follows: Chapters \ref{Chapter2} and \ref{Chapter3} review 2 related methods that are highly relevant to our current work. \cref{Chapter2} describes the {\tt SE} edge detector which trains a decision forest to model the relationship between a local image patch and its segmentation. \cref{Chapter3} highlights the edge detector and segmentation machinery {\tt gPb-OWT-UCM}. Despite the segmentation pipeline {\tt OWT-UCM} demanding an oriented boundary signal, it is currently a widely applied post-processing step to allow edge detectors to ``close'' contours and provide segmentation. In \cref{Chapter4} we describe our proposed algorithm for obtaining a hierarchical segmentation from extracted edges by using only local information. Our experiments and evaluation are in \cref{Chapter5}. We finish by summarising our findings and highlighting open questions that might be a basis for future investigation. %searchings.
