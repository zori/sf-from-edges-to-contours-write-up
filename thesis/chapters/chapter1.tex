...
Both are %That is 
easily possible by using a datastructure called {\it Ultrametric contour map} ({\bf UCM})~\cite{Arbelaez2006boundary}. While details will be given in \sref{sec:ch3-UCM}, for now it is sufficient to know that it encapsulates a hierarchy of segmentations. 
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.8\textwidth]{images/related_work_table.pdf} % OMG, that looks so much better and compiles faster; if possible, export as pdf from now on
