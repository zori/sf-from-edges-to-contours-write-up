...
\label{Chapter1}
\section{Motivation}
Recently, very good edge detection results were achieved using random decision forests~\cite{DollarICCV13edges}. While a certain edge model is % required, needed % a requirement
imperative, few segmentation algorithms use edge detection output as a first step to image segmentation. Consequently, many edge detection algorithms re-use the same approach to obtain image segmentation in addition %additionally 
to their edge detection.

Therefore, the field is limited by lack of research on the problem of edge-based hierarchical segmentation. We investigate an alternative % possibility 
...
\centering
 ...
 BSDS500~\cite{BSDS500resources}]{%
  \includegraphics[width=0.4\textwidth]{images/examples/koala/koala.jpg} % hawaii/arbelaez2011-035.png}
 }
 \subfigure[Edge map]{%
  \includegraphics[width=0.4\textwidth]{images/examples/koala/koala_gPb_edge_map.png} % too thin - SE are thicker koala_SE_edge_map % hawaii/edge_map_arbelaez2011-039.png}
  \label{fig:sub:edge_detection-edge-map}
 }
 \subfigure[Probability of boundary]{%
  \includegraphics[width=0.4\textwidth]{images/examples/koala/koala_gPb.png} % koala_SE % hawaii/Pb_arbelaez2011-039.png}
  \label{fig:sub:edge_detection-pb}
 }
 \subfigure[Probability of boundary jet colour-coded]{%
  \includegraphics[width=0.4\textwidth]{images/examples/koala/koala_gPb_cc.png} % koala_SE_cc % hawaii/Pb_arbelaez2011-039.png}
...
the negative of the probability and the edge map are shown, \ie the darker the pixel, the higher the probability of an edge. Visualisation code for \protect\subref{fig:sub:edge_detection-pb-cc} courtesy of Hallman and Fowlkes~\cite{Hallman2014}.}
...
In \fref{fig:edge_detection} we show the result of the edge detection algorithm gPb~\cite{Arbelaez11}. The last two images show the detector output - a {\it Probability of boundary}. For improved viewing in case of probabilistic output we will utilise a visualisation with black as background and heatmap-coloured boundaries according to their strength. The edge map \fref{fig:sub:edge_detection-edge-map} was obtained by thresholding at $t=0.3$.
