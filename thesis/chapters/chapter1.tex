...
\textbf{Edge map:} We will call an \textit{edge map} a binary image depicting edges locations. The pixels belonging to an edge are set to 1. The rest of the pixels are not part of an edge. They have a value of 0. Therefore, for a given pixel location $(x,y)$, the corresponding edge map value is $em_{x,y} \in \{0,1\}$ (see \fref{fig:edge_detection-sub2}).

\textbf{Probability of boundary:} To allow to express a level of (un)certainty as to the presence of an edge in an image, we employ a probabilistic means. We define a real-valued image called a \textit{Probability of boundary (Pb)} (see \fref{fig:edge_detection-sub3}). 
