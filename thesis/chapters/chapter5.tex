...
\settocdepth{section}
The benchmark that we use provides, among others, two precision-recall metrics - one boundary- and one region-oriented. % a boundary and a region oriented one.
...
local change in the boundary on the score:} A difference in the score of a single region boundary pixel should not greatly affect the edge detector output. Therefore, it correctly has only a small impact on the BPR metric. For the task of image segmentation however, a change in the strength of a single pixel could result in merging neighbouring regions. A ``weaker'' pixel among a strong intervening boundary could be thought of as a leakage, which will cause the {\tt UCM} procedure % algorithm 
(featured in \sref{sec:ch3-UCM}) % TODO give the algorithm in an appendix
...
As a baseline, we create a hierarchy of segmentations on top of the {\tt SE} detector result. This is in the spirit of \cite{Arbelaez2006boundary} who, however, use the edge detector {\tt Pb} which we described in \sref{sec:ch3-Pb}. 

What we do for this baseline could also be understood %thought 
...
We then build the multiscale hierarchy - {\tt UCM}, for a description see \sref{sec:ch3-UCM}. Benchmark plots are on \fref{fig:SE-watershed}.
...
We have chosen to compare here four methods and their corresponding oracles. \fref{fig:oracle} contains their precision-recall quantitative results and \tref{tab:oracle} all the measures that we report. Each method uses a different {\bf patch transformation} and a different {\bf scoring function} than the others: 
\begin{enumerate}
