...
\caption[{\tt OEF} decision forest predicts a probability distribution over a fixed patch space]{{\tt OEF} decision forest predicts a probability distribution over a fixed patch space (given on the left). Images courtesy of~\cite{Hallman2014}}
...
While the {\tt SE} algorithm is a state-of-the-art edge detector, it does not provide a segmentation. One still need to ``connect'' the resulting edges to extract complete contours. % that seem so obvious for the human eye+mind.
