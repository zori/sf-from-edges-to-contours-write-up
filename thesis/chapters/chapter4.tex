...
finely partition the image pixel graph (see \sref{sec:ch3-watershed} for a description of the \textit{watershed transform} which we will use). The regions closed by them constitute an oversegmentation of the image. 
...
By reasoning about the salience of region boundaries, one could construct a \textit{hierarchy of segmentations} (see \sref{sec:ch3-UCM} for details on the {\tt UCM}). Going up the hierarchy corresponds to coarsening the segmentation by merging neighbouring regions which are separated by a weak boundary. To establish the order in which to merge the regions, we must be able to tell weak from strong boundaries. We want to associate a \textit{score} with every possible region boundary, which %score 
should reflect the strength of that intervening boundary.

The {\tt OWT} reviewed in \sref{sec:ch3-OWT} provided one possible way to obtain such a score, by using oriented edge information. In the following we propose a different methodology which relies on patch comparison and local edge structure.
...
Beside being heavily-engineered, leveraging such hand-crafted cues, in terms of detection speed it does not come close to the fast speed %real-time performance 
of {\tt SE}.

Our chief reason for using {\tt SE} 
% The chief reason for using {\tt SE} for us, however, 
...
To cast a single vote %To this end, 
...
\end{center}
\caption[Patches examples - watershed and structured decision tree]{Examples of {\bf watershed} patch segmentations (colour-coded on the {\bf left}; white pixels mark the explicit boundary) and structured decision {\bf tree leaf} patch segmentations (greyscale on the {\bf right}).}
...
It aims to merge superpixels into segments according to the structured tree patch (\fref{fig:sub:ws-greedy-merge-tree-leaf}), while respecting the condition that the central watershed pixel lie on a boundary between two regions. % NOTE suggested by Mathieu; not ``lies''!!!
% It %uses heuristic to
% aims for an optimal superpixels merge into segments, according to the structured tree patch (\fref{fig:sub:ws-greedy-merge-tree-leaf}). At the same time it still respects the condition that the central watershed pixel lies on a boundary between two regions. 
\fref{fig:sub:ws-greedy-merge-ws-fairer-greedy-merge} shows an example.
...
See \fref{fig:ws-just-contour} for an example. Here we take into account only the underlying region boundary, on which the central pixel of the watershed patch is found. % is.
...
\textbf{Tree leaf segmentation transformation:} As the resulting watershed patch is not a segmentation --- often it would contain just an edge, and not a contour --- we cannot keep the tree leaf as segmentation labelling (see \fref{fig:sub:tree-leaf-segments}. % NOTE for the triple ``---'' see http://tex.stackexchange.com/questions/3819/dashes-vs-vs
Instead we transform it, making the boundaries between the regions explicit, as in \fref{fig:sub:tree-leaf-boundaries}.
...
\subparagraph{Line fitted through arc {\bf end points}:}
...
\textbf{Region boundary subdivision into watershed arc:} Note that the first two approaches only use the end points, so it is important for their proper functioning that the arcs have a ``consistent orientation'', \ie  can be well approximated by a line. % \ie, can well be approximated by a line. 
...
In the following we will use $S$ to denote the machine boundary map, $\mathbb{S}$ for the machine segmentation, $s$ for segment in it, $G$ for the ground-truth boundary map, $\mathbb{G}$ for the ground-truth segmentation, and $g$ for segment in it. % many tiny suggestions by Mathieu
...
It is based on {\bf boundary matching} - it matches the edges detected by the algorithm under consideration %test 
...
% mention that in implementation it is an approximation, otherwise - n^3 cubic in the number of pixels in the image (or in the boundaries)
For the purposes of benchmarking, this metric is implemented as an approximation. Otherwise the cubic in the number of pixels %in the image
theoretical complexity would prove to be prohibitive for computation.

An advantage of this  metric is that it is sensitive to the complexity of the boundary. In addition, it can be applied % is applicable 
...
Even though the PASCAL challenge includes multiple object classes, the evaluation is done on a per-class basis. Hence, the above metric is practically the {\it Overlap} for the two regions - one in the machine segmentation, and the other in the ground truth segmentation. %one. 
% TODO discuss the problem that requires normalisation
Considering one class at a time, in a one-vs-all fashion, poses the problem as a binary classification problem. Thus normalisation is naturally provided, as the number of classes is fixed to two. %2.
...
The above erroneously penalises two equal segmentations (perfect match) just for having a higher number of segments. %larger amount of segments.
\end{enumerate}
Both proposals % Both our proposals 
...
This %In the way just described 
...
Of the above metrics, BPR, RI, and not-normalised VPR are symmetric \wrt the segmentation being tested %under test 
...
that for normalised VPR we have considered the transformed \textbf{watershed patch} to be the \textbf{segmentation being tested %under test 
($\mathbb{S}$)} and the other patch to be a ground truth ($\mathbb{G}_i$). 

In most experiments (our {\tt Structured voting}; to be comprehensively % systematically detailedly 
reviewed in \sref{sec:ch5-structured-voting}) the ``other patch'' is a segmentation patch from a decision tree leaf, the estimated % - one predicted as 
most likely segmentation for the location. In another series of experiments (termed {\it oracle experiments}, as we use the ground truth; detailed in \sref{sec:ch5-oracle}) the ``other patch'' being compared is a crop from the ground truth annotated segmentations. 

It is then clear that the watershed patch, since it is always a {\bf single patch}, cropped from the oversegmentation image, receives this allocation (the segmentation being tested $\mathbb{S}$ is also {\bf exactly one} %at a time
...
for the benchmark task to have the output values of both edge detector and image segmentation lie in the interval %into 
$[0,1]$. That additionally lends itself to a probabilistic interpretation, which could potentially facilitate the combination of outputs of different models. %allow to easily combine different models.
