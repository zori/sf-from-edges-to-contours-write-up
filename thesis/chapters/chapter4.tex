...
See fig.~\ref{fig:ws-just-contour} for an example. Here we take into account only the underlying region boundary, on which the central pixel of the watershed patch is.
...
\textbf{Watershed arc:} We recursively subdivide this boundary until a \textbf{watershed arc} is obtained, fig.~\ref{fig:sub:ws-just-contour-watershed-just-contour}, whose shape could be sufficiently accurately approximated by the straight line through the arc's end points. See fig.~\ref{fig:Arbelaez11-contour-subdivision}. The procedure is as first described in ``Contour Detection and Hierarchical Image Segmentation''~\cite{Arbelaez11}.

\textbf{Tree leaf segmentation transformation:} As the resulting watershed patch is not a segmentation - often it would contain just an edge, and not a contour, we cannot keep the tree leaf as segmentation labelling (see fig.~\ref{fig:sub:tree-leaf-segments}. Instead we transform it, making the boundaries between the regions explicit, as in fig.~\ref{fig:sub:tree-leaf-boundaries}.

\textbf{Edge detection comparison:} Consequently, we would need a boundary evaluation metric to compare the two, as the watershed patch, when transformed, won't necessarily contain regions. Details on the metric that we will use are given in~\textsection~\ref{sec:ch4-BPR-maths}.
...
We utilise 3 types of line fitting, shown on figures~\ref{fig:ws-line-fitting} and~\ref{fig:ws-line-fitting2}.
...
\textbf{Region boundary subdivision into watershed arc:} Note that the first two approaches only use the end points, so it is important for their proper functioning that the arcs have a ``consistent orientation'', \ie, can well be approximated by a line. In~\textsection~\ref{Chapter5} we conduct experiments to determine whether it is of any importance to use watershed arcs instead of the whole region boundary.
...
\begin{center}
 ...
 \subfigure[Line fitted through arc {\bf end points}]{
  ...
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-line-fitting/ends-seg.png}
 ...
 \subfigure[Line fitting through {\bf centre} based on arc end points]{
  ...
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-line-fitting/centre-seg.png}
 ...
 \subfigure[Line estimated with {\bf linear least squares}]{
  ...
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-line-fitting/lls-seg.png}
  \label{fig:sub:ws-line-fitting-lls}
  }
\end{center}
\caption[First example of watershed {\bf line fitting}]{First example of watershed line fitting.}
...
\begin{center}
 ...
 \subfigure[Line fitted through arc {\bf end points}]{
  ...
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-line-fitting/ends-seg2.png}
 ...
 \subfigure[Line fitting through {\bf centre} based on arc end points]{
  ...
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-line-fitting/centre-seg2.png}
 ...
 \subfigure[Line estimated with {\bf linear least squares}]{
  ...
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-line-fitting/lls-seg2.png}
...
In fig.~\ref{fig:ws-quadratic-fitting} we show 6 examples of quadratic fitting. We have omitted the source image % leopard and hawaii
...
\begin{center}
  ...
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-quadratic-fitting/quadr-lls-seg1.png} &
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-quadratic-fitting/quadr-lls-seg2.png} &
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-quadratic-fitting/quadr-lls-seg3.png} &
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-quadratic-fitting/quadr-lls-seg4.png} &
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-quadratic-fitting/quadr-lls-seg6.png} &
  \includegraphics[width=0.13\textwidth,frame]{images/SE-SV-UCM/ws-quadratic-fitting/quadr-lls-seg5.png} \\

  ex. 1 & ex. 2 & ex. 3 & ex. 4 & ex. 5 & ex. 6 \\
  \end{tabular}
\end{center}
\caption[Watershed quadratic fitting]{Watershed quadratic fitting. For each of the 6 examples, the {\bf first} row features the cropped watershed patch. The {\bf second} row has the relevant watershed arc. The {\bf third} row has the result from the {\bf quadratic fitting} (boundary is explicit). On the {\bf fourth} row is the segmentation labelling - the final transformed watershed patch which we use to compare to the structured forest segmentations.}
...
The result from the SV - a weighted watershed locations image, is suitable for input to the UCM algorithm. The UCM builds a hierarchy of segmentations by agglomerative clustering of regions based on the strength of their separating boundary. We reviewed UCM in~\ref{sec:ch3-UCM}. By thresholding the UCM we can obtain segmentations at different level of detail. The boundaries of the regions in a segmentation constitute contours, \ie, they are ``closed'' edges. This makes our algorithm suitable for evaluation by edge detector as well as image segmentation benchmarks.
